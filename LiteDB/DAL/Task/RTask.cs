...
namespace KVM.LiteDB.DAL.Task
{
  public class RTask : RBase<entity.Task>, ITask
  {
    ...
    public new entity.Task GetOne(string id)
    {
      ...
      Task.HoleGroupsRadio = _holeGroup.Find(o => o.ParentId == id).Select(h => new HoleGroupsRadio { Hole = h.Name, Id = h.Id });
    ...
    public IEnumerable<TaskMenu> MenuData(string projectId)
    {
      return _col.Find(t => t.ProjectId == projectId)
        .GroupBy(p => (p.ComponentName, p.ComponentId), (key, holeGroup) => new TaskMenu { Name = key.ComponentName, Id = key.ComponentId, Count = holeGroup.Count()});
    ...
    {
      return _col.Find(t => t.ProjectId == projectId && t.ComponentId == componentId).Select(x => new MenuData {Name = x.BridgeName, Id = x.Id });
    }
  }
}
