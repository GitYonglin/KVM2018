...
namespace MODBUSSocket
{
    ...
    public partial class ASCII: SocketLink
    {




        public ASCII(string Ip, int Port) : base(Ip, Port)
        {
        }
        ...
        {
            SendData sendData = MODBUS.ASCIISend.F03(ipAddress, address, data);
        ...
        {
            SendData sendData = MODBUS.ASCIISend.F05(ipAddress, address, data);
            AddListSand(new ListSendData { SendData = sendData, Receive = callback });
        }
        public void F06(int ipAddress, int address, int data, Receive callback)
        {
            SendData sendData = MODBUS.ASCIISend.F06(ipAddress, address, data);
            AddListSand(new ListSendData { SendData = sendData, Receive = callback });
        }
        public void F16(int ipAddress, int address, int[] data, Receive callback)
        {
            SendData sendData = MODBUS.ASCIISend.F16(ipAddress, address, data);
            AddListSand(new ListSendData { SendData = sendData, Receive = callback });
        }
